           ...
           # imagex =self.Bed_image()
            for i in range(0,len(et.getroot()[j])):
               # shape_type = et.getroot()[j][i].attrib  # for extracting attribss
                if j ==1:
                  self.z1 = et.getroot()[j].get('{http://slic3r.org/namespaces/slic3r}z')
                if j ==2:
                   self.z2 = et.getroot()[j].get('{http://slic3r.org/namespaces/slic3r}z')
                   self.setZdiff(self.z2,self.z1)
...
#               
    ...
    def setZdiff(self,z2,z1):
        self.Zdiff = (float(z2)-float(z1) )*1000000
        #print(self.Zdiff)
    def getZdiff(self):
        return self.Zdiff
...
#        start = time.time()
#        ia =im.poly2imgWrite(im.getlayer_data(0),im.get_totalLayer_shape(0))
#        np.savetxt(loc+"\Image"+str(0) +".txt",ia, fmt = '%d',delimiter=',',newline ='\n',header ='[',footer =']')
#        end1 = time.time()
...
#    print("time taken" + str(end-start)) // la7d hena
    #print(im.Bed_image())
